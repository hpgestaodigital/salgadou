...
type NavItem = { href: string; label: string; icon: NavIcon; modulo: Modulo | "producao" | "todos" }
...
const NAV: NavItem[] = [
  ...
  { href: "/cmv", label: "CMV por Produto", icon: Calculator, modulo: "cmv" },
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function carregarSessao(user: import("@supabase/supabase-js").User | null) {
      if (!ativo) return
      if (!user) return setSessao(null)
      const permissoes = await carregarPermissoes(user)
      if (!ativo) return
      setSessao({ nome: getNome(user), papel: getPapel(user), admin: isAdmin(user), avatarUrl: String(user.user_metadata?.avatar_url ?? ""), permissoes })
    }
    supabase.auth.getUser().then(({ data }: { data: { user: import("@supabase/supabase-js").User | null } }) => carregarSessao(data.user))
    const { data: sub } = supabase.auth.onAuthStateChange((_event: AuthChangeEvent, session: Session | null) => void carregarSessao(session?.user ?? null))
  ...
  const itens = NAV.filter((item) => {
    ...
    if (item.modulo === "todos") return true
    if (item.modulo === "producao") return Boolean(sessao.permissoes.producao_compras || sessao.permissoes.producao_estoque || sessao.permissoes.producao_planejamento)
    ...
    <div className="flex min-h-screen bg-background">
      ...
      {menuAberto && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="absolute left-0 top-0 flex h-full w-72 flex-col border-r border-sidebar-border bg-sidebar text-sidebar-foreground shadow-2xl">
            <div className="flex justify-end p-2"><Button variant="ghost" size="icon" onClick={() => setMenuAberto(false)} aria-label="Fechar menu"><X className="size-5" /></Button></div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 flex items-center gap-3 border-b border-border/70 bg-background/90 px-4 py-3 backdrop-blur-xl lg:hidden">
          ...
          <div><p className="text-[10px] font-bold uppercase tracking-[0.24em] text-primary">Salgadou Gestão</p><p className="text-xs text-muted-foreground">Painel interno</p></div>
        </header>
        {pathname !== "/" && <div className="mx-auto w-full max-w-[1480px] px-4 pt-3 sm:px-6 lg:px-8 lg:pt-4 xl:px-10"><GlobalGoalsBar /></div>}
        <main className={cn("mx-auto w-full max-w-[1480px] flex-1 p-4 sm:p-6 lg:p-8 xl:p-10", pathname !== "/" && "pt-4 sm:pt-5 lg:pt-6 xl:pt-6")}>{children}</main>
      </div>
    </div>
  )
}

function SidebarContent({ pathname, itens, sessao, logoUrl, onSair, onNavigate }: { pathname: string; itens: NavItem[]; sessao: SessaoUI; logoUrl: string; onSair: () => void; onNavigate?: () => void }) {
  return (
    <>
      <div className="flex items-center gap-3 px-5 py-7">
        <span className="grid size-11 place-items-center overflow-hidden rounded-full bg-primary text-lg font-heading font-black text-primary-foreground">{logoUrl ? <img src={logoUrl} alt="Logo da Salgadou" className="size-full object-cover" /> : "S"}</span>
        <div className="leading-tight"><p className="font-heading text-xl font-extrabold tracking-tight">Salgadou</p><p className="text-xs text-sidebar-foreground/60">Gestão Interna</p></div>
      </div>
      <nav className="flex flex-1 flex-col gap-1.5 overflow-y-auto px-3 py-2">
        {itens.map((item) => {
          const ativo = item.href === "/" ? pathname === "/" : pathname.startsWith(item.href)
          const Icon = item.icon
          return <Link key={item.href} href={item.href} onClick={onNavigate} aria-current={ativo ? "page" : undefined} className={cn("flex items-center gap-3 rounded-xl px-3.5 py-3 text-sm font-semibold outline-none transition-colors", ativo ? "bg-[#352a25] text-[#fff7ef]" : "text-[#eee8e2] hover:bg-white/[0.055] hover:text-[#fffaf5]")}><Icon className="size-[18px] shrink-0" aria-hidden /><span>{item.label}</span></Link>
        })}
      </nav>
