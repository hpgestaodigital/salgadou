...
export function SauceStockControl() {
  ...
  async function registrarProducao() {
    ...
    setProducao({ ficha_id: "", receitas: "1", grandes: "", pequenas: "", data: hoje, observacoes: "" }); await carregar()
  }

  async function registrarSaida() {
    if (!saida.ficha_id || Number(saida.quantidade) <= 0) return toast.error("Informe molho e quantidade.")
    setSaving(true)
    ...
    setSaida({ ficha_id: "", tamanho: "grande", quantidade: "", motivo: "vendido", data: hoje, observacoes: "" }); await carregar()
  }
  ...
  return <div className="space-y-5">
    <div className="grid gap-4 lg:grid-cols-2">
      <Card className="border-primary/25"><CardHeader><CardTitle>Produção de molho</CardTitle><p className="text-sm text-muted-foreground">Baixa os ingredientes da ficha e cria lotes de bisnagas.</p></CardHeader><CardContent className="grid gap-3 md:grid-cols-2">
        ...
        <div><Label>Receitas</Label><Input type="number" step="0.1" value={producao.receitas} onChange={(e) => setProducao({ ...producao, receitas: e.target.value })} /></div><div><Label>Data</Label><Input type="date" value={producao.data} onChange={(e) => setProducao({ ...producao, data: e.target.value })} /></div>
        <div><Label>Bisnagas grandes</Label><Input type="number" value={producao.grandes} onChange={(e) => setProducao({ ...producao, grandes: e.target.value })} /></div><div><Label>Bisnagas pequenas</Label><Input type="number" value={producao.pequenas} onChange={(e) => setProducao({ ...producao, pequenas: e.target.value })} /></div>
      ...
      <Card><CardHeader><CardTitle>Saída de molho</CardTitle><p className="text-sm text-muted-foreground">Venda, perda, vencimento, consumo interno ou ajuste com baixa FIFO.</p></CardHeader><CardContent className="grid gap-3 md:grid-cols-2">
        ...
        <div><Label>Tamanho</Label><select className={selectClass} value={saida.tamanho} onChange={(e) => setSaida({ ...saida, tamanho: e.target.value })}><option value="grande">Grande</option><option value="pequena">Pequena</option></select></div><div><Label>Quantidade</Label><Input type="number" value={saida.quantidade} onChange={(e) => setSaida({ ...saida, quantidade: e.target.value })} /></div>
    ...
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{estoques.map((e) => <Card key={e.ficha_id}><CardHeader><CardTitle>{e.nome}</CardTitle></CardHeader><CardContent className="flex gap-2"><Badge>Grandes: {e.grandes_disponiveis}</Badge><Badge variant="secondary">Pequenas: {e.pequenas_disponiveis}</Badge></CardContent></Card>)}</div>

    <Card><CardHeader><CardTitle>Últimas movimentações de molho</CardTitle></CardHeader><CardContent className="space-y-2">{movimentos.length === 0 ? <p className="text-sm text-muted-foreground">Nenhuma movimentação.</p> : movimentos.map((m) => <div key={m.id} className="flex flex-wrap justify-between gap-2 rounded-lg border p-3 text-sm"><div><strong>{fichas.find((f) => f.id === m.ficha_id)?.nome ?? "Molho"}</strong><p className="text-muted-foreground">{m.motivo} · {m.tamanho}</p></div><div className="text-right"><strong>{m.tipo === "saida" ? "-" : "+"}{m.quantidade}</strong><p className="text-muted-foreground">{new Date(m.data_movimentacao + "T12:00:00").toLocaleDateString("pt-BR")}</p></div></div>)}</CardContent></Card>
  </div>
}
